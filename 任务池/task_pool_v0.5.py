...
"""
已经分配的任务在一定时间内自动清除，可以在任务池中添加一个定期清理的机制。
"""
...
class TaskPool:
    ...
    def cleanup_assigned_tasks(self):
        with self.lock:
            current_time = time.time()
            if current_time - self.last_cleanup_time > self.cleanup_interval:
                self.tasks = [(score, task) for score, task in self.tasks if task.status != "Assigned"]
